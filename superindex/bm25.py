...
Only raw counts are stored; IDF and the average page length are computed at
query time over the documents being searched, so a cross-document search
ranks all pages on one scale. A store written before this index existed is
indexed lazily on first search (and on the next `index` run).
...
def search(store_path: str | os.PathLike[str], query: str, *,
           doc_ids: list[str] | None = None, top_k: int = 5) -> SearchResult:
    ...
    n_pages = sum(sum(1 for n in ix["lengths"] if n) for ix in indexes.values())
    total = sum(sum(ix["lengths"]) for ix in indexes.values())
    if not n_pages:
        return SearchResult([], len(metas), built)
    avgdl = total / n_pages

    scores: dict[tuple[str, int], float] = {}
    for term, qtf in terms.items():
        df = sum(len(ix["postings"].get(term, ())) // 2 for ix in indexes.values())
        if not df:
            continue
        idf = math.log(1 + (n_pages - df + 0.5) / (df + 0.5))
        for doc_id, ix in indexes.items():
            posting = ix["postings"].get(term)
            if not posting:
                continue
            lengths = ix["lengths"]
            for j in range(0, len(posting), 2):
                page, tf = posting[j], posting[j + 1]
                norm = tf + K1 * (1 - B + B * lengths[page - 1] / avgdl)
                key = (doc_id, page)
    ...
    _decorate(store, hits, query)
    return SearchResult(hits, len(metas), built)


def _decorate(store: DocStore, hits: list[Hit], query: str) -> None:
    """Section titles from the tree, snippets from the page text."""
    ...
    for hit in hits:
        ...
        heading = _heading_before(text, _best_window(text, pattern, SNIPPET_CHARS))
        hit.section, hit.node_id = section_for(trees[hit.doc_id], hit.page, heading)
        hit.snippet = snippet(text, pattern)
