"""Public API for SuperIndex Flash: :func:`page_index_flash` builds the tree, :func:`flash_rejection_reason` is the refusal policy the local client and CLI share. Everything else in this package is internal pipeline machinery."""
...
def flash_rejection_reason(result: dict, standard_hint: str = "mode='standard'") -> str | None:
    ...
    if result.get("toc_source") == "unreadable":
        return ("SuperIndex Flash found no text layer in this PDF (scanned or "
                "image-only); run OCR before indexing it.")
    if result.get("toc_source") == "pages" and len(structure) > FLAT_TREE_MAX_NODES:
        return (f"SuperIndex Flash found no layout structure in this document "
                f"({len(structure)} pages); try {standard_hint}, which builds "
                "the structure with the model.")
    if not structure:
        return ("SuperIndex Flash could not extract a structure from this PDF; "
