"""Local implementation of the SuperIndex SDK surface."""
...
class LocalAPI:
    ...
    def submit_document(
        ...
        try:
            ...
            raise SuperIndexAPIError(f"Failed to submit document: {e}") from e
    ...
    def _index_flash(self, file_path: str) -> tuple[list, str | None]:
        ...
        if reason:
            raise SuperIndexAPIError(f"Failed to submit document: {reason}")
    ...
    def _require_data(data, error_prefix: str):
        if data is None:
            raise SuperIndexAPIError(f"{error_prefix}: stored document data is unreadable.")
        return data
    ...
    def _require_doc(self, doc_id: str, error_prefix: str) -> dict:
        ...
        if meta is None:
            raise SuperIndexAPIError(f"{error_prefix}: Document not found.")
        return meta

    def get_document(self, doc_id: str) -> dict[str, Any]:
        meta = self._store.get_meta(doc_id)
        if meta is None:
            raise SuperIndexAPIError("Failed to get document metadata: Document not found")
    ...
    def delete_document(self, doc_id: str) -> dict[str, Any]:
        if not self._store.delete_document(doc_id):
            raise SuperIndexAPIError("Failed to delete document: Document not found.")
