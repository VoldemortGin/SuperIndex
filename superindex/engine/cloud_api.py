"""Cloud mode (the hosted PageIndex service, VectifyAI), based on the 0.2.8 client."""
import requests
...
class CloudAPI:
    """
    Python SDK client for the PageIndex cloud API.
    """
    ...
    def get_page_image(self, doc_id: str, page: int) -> str:
        ...
        for img in response.json().get("images") or []:
            if img.get("page") == page and img.get("url"):
                return img["url"]
        raise SuperIndexAPIError(f"No image URL returned for page {page}.")
    ...
    def get_document_image(self, doc_id: str, img_id: str) -> str:
        ...
        if not url:
            raise SuperIndexAPIError(f"No image URL returned for {img_id!r}.")
        return url
    ...
    def submit_query(self, doc_id: str, query: str, thinking: bool = False) -> Dict[str, Any]:
        """
        Submit a retrieval query for a specific PageIndex cloud document.
    ...
    def chat_completions(
        ...
        """
        PageIndex cloud Chat Completions. Optionally scoped to specific PageIndex cloud documents.
    ...
    def delete_document(self, doc_id: str) -> Dict[str, Any]:
        """
        Delete a PageIndex cloud document and all its associated data.
