...
* `AZURE_DI_ENDPOINT` **and** `AZURE_DI_KEY` both set  ->  **Azure Document
  ...
  `scripts/02_qa_test.py` (PageIndex indexing) and `nav/build.py` (the two-level
  navigator). Nothing else needs to be passed on the command line.
* Otherwise -> the built-in text-layer path (PyPDF2, exactly what PageIndex's
...
two callers want different views of it: PageIndex wants per-page text, `nav`
...
def describe(env: Optional[dict[str, str]] = None) -> BackendInfo:
    ...
    return BackendInfo(
        BACKEND_TEXT_LAYER,
        "PDF text layer via PyPDF2 (PageIndex default). "
...
class Extractor:
    ...
    def page_texts(self, pdf: "str | Path") -> list[str]:
        """Per-page plain text. Drop-in for PageIndex's `_extract_page_texts`."""
        pdf = Path(pdf)
        if self.uses_azure:
...
def _pypdf2_page_texts(pdf: Path) -> list[str]:
    """Delegate to PageIndex's own extractor so the fallback is byte-identical."""
    try:
        from pageindex.local_api import LocalAPI
        return LocalAPI._extract_page_texts(str(pdf))
    except Exception:  # noqa: BLE001 - PageIndex not importable; inline copy
        import PyPDF2
...
def install_into_pageindex(env: Optional[dict[str, str]] = None,
    ...
    """Make PageIndex's local mode use the active backend.

    PageIndex reads the PDF text layer in `LocalAPI._extract_page_texts`, a
    staticmethod returning one string per page. We swap in our own callable
    that returns the same shape, so nothing inside `PageIndex/` is edited and
    ...
    if verbose:
        announce(env)
        print("          → 已接管 PageIndex 的 PDF 文本抽取（_extract_page_texts）")
    return info
